...
export default function ExplorePage() {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-ink pt-16">
      ...
      <aside className={`absolute right-0 top-16 bottom-0 z-20 transition-all duration-500 ease-in-out ${isSidebarOpen ? 'w-[450px]' : 'w-0 overflow-hidden'}`}>
        <div className="h-full glass-panel border-l border-white/5 flex flex-col">
          <div className="p-8 space-y-6">
            <div className="space-y-2">
              ...
              <p className="text-xs text-paper/40 tracking-[0.2em] font-light">FOUND {filtered.length} ARCHITECTURAL GENES</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-8 pb-12 custom-scrollbar">
            <div className="space-y-6">
              {loading && <div className="py-10 text-paper/30 text-xs tracking-widest uppercase">Loading buildings...</div>}
      ...
      <button
        ...
        className={`absolute z-30 top-1/2 -translate-y-1/2 transition-all duration-500 flex items-center justify-center ${isSidebarOpen ? 'right-[450px]' : 'right-0'} w-6 h-20 bg-ink/60 backdrop-blur border border-white/10 text-paper/40 hover:text-paper hover:bg-ink rounded-l-md`}
      >
        <span className="text-[10px] transform -rotate-90 whitespace-nowrap tracking-[0.3em]">{isSidebarOpen ? 'CLOSE' : 'LIST'}</span>
      </button>
